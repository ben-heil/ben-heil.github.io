...
If you leave the beaten path, you can see more creative views of science publication as well.
...
#### Peer review
Although I am a proponent of the preprint system, I do believe that peer review has value.
The issue is that its current form is inefficient, and misaligned with the goals of science.
The current incarnation of peer review asks "is this paper good[^good] enough to be published in this journal", while it should ask "_how_ good is this paper?"
...
TODO code review (industry vs academia)


In my [recent paper](https://www.nature.com/articles/s41592-021-01256-7) we[^we] proposed having a collaborator or coauthor whose entire role is to make sure the work is reproducible.
More eloquently:
> We envision a reproducibility collaborator as someone outside the primary authors’ research groups who certifies that they were able to reproduce the results of the paper from only the data, models, code and accompanying documentation. 
...
However, we can do much better than "wow, this paper would have cost a lot to print, that figure has five colors!" by [making papers interactive](https://distill.pub/).
...
Why use only 2D figures when you could make a 3D figure that can rotate in your browser?
